...
def show_educational_comments():
    """Streamlit components explaining visualization concepts"""
    with st.expander("💡 Why proper visualization matters"):
        st.markdown("""
        **Common Pitfalls in Scientific Visualization:**
        1. **Axis Misalignment:** Numerical arrays often use (x,y) indexing while visualization libraries expect (y,x)
        2. **Boundary Swapping:** Incorrect coordinate mapping can flip top/bottom boundaries
        3. **Data Orientation:** Matrix rows/columns might not match physical coordinates
        """)
    
    with st.expander("⚠️ Critical Visualization Checklist"):
        st.markdown("""
        Always verify:
        1. Boundary values match physical constraints
        2. Axis directions correspond to real-world coordinates
        3. Color gradients align with expected diffusion patterns
        """)
...
# ========== Visualization Cases ==========
@st.cache_data
def evaluate_case(_model, mode, times, Lx, Ly):
    """Handle data processing for different visualization cases"""
...
def create_visualization(mode, X, Y, c1_list, c2_list, times, Lx, Ly, time_index):
    """Generate visualization for selected case with time slider"""
    # Coordinates
...
if __name__ == "__main__":
    st.title("PINN Visualization: Case Study")
    st.markdown("""
    Compare three visualization approaches:
    1. **Case-I (C):** Incorrect axes mapping
    2. **Case-II (A):** Flipped y-axis
    3. **Case-III (B):** Correct implementation
    """)
    
    # Show educational comments
    show_educational_comments()
    ...
    case = st.radio("Select visualization case:", 
                   ["Case-I (C)", "Case-II (A)", "Case-III (B)"])
    
    # Fixed diffusion coefficients
    D11, D12 = 1e-4, 5e-5
    D21, D22 = 5e-5, 1e-4
    
    # Validate T_max and create times array
    if T_max <= 0:
        st.error("Simulation time must be positive!")
        st.stop()
    times = np.linspace(0, T_max, 20)
    
    if st.button("Train Model & Visualize"):
        ...
        with st.spinner("Evaluating model..."):
            X, Y, c1_preds, c2_preds, boundary_checks = evaluate_case(
                st.session_state.model, case, times, Lx, Ly
            )
            # Store results in session state to avoid re-computation
    ...
    if 'eval_results' in st.session_state:
        ...
        X = eval_results['X']
        Y = eval_results['Y']
        c1_preds = eval_results['c1_preds']
        c2_preds = eval_results['c2_preds']
        boundary_checks = eval_results['boundary_checks']
        ...
        # Case-specific educational notes
        with st.expander("What's happening here?"):
            if case == "Case-I (C)":
                st.error("""
                **Flaws:**\n
                1. Data not transposed - Plotly uses (y,x) convention
                2. BCs appear on sides instead of top/bottom
                3. Axis labels don't match physical coordinates
                """)
            elif case == "Case-II (A)":
                st.warning("""
                **Partial Fix:**\n
                1. Data transposed to (y,x)
                2. But y-axis reversed unnecessarily
                3. BCs flipped vertically
                """)
            else:
                st.success("""
                **Correct Approach:**\n
                1. Data transposed to (y,x)
                2. Natural y-axis direction
                3. Boundaries match physical constraints
                """)
        
        # Visualize for selected time
        create_visualization(case, X, Y, c1_preds, c2_preds, times, Lx, Ly, time_index)
        
        # General educational notes
        st.markdown("---")
        with st.expander("💡 Key Lessons"):
            st.markdown("""
            **Critical Visualization Principles:**
            1. **Data Alignment:** Match array axes to visualization library expectations
            2. **Axis Direction:** Verify natural coordinate system alignment
逗            3. **Boundary Checks:** Always validate edge values numerically
            4. **Transposition:** Essential when using `meshgrid(indexing='ij')`
            """)
