...
class DualScaledPINN(nn.Module):
    def __init__(self, D11, D12, D21, D22, Lx, Ly, T_max):
        ...
        self.cu_head[2].weight.data.fill_(2.85e-3)
        self.ni_head[2].weight.data.fill_(9.2e-4)
...
def boundary_loss_bottom(model):
    ...
    c_pred = model(x, y, t)
    return (torch.mean((c_pred[:, 0] - 2.85e-3)**2) + 
...
def plot_losses(loss_history, Ly, output_dir):
    ...
    plt.title(f'Training Loss for Ly = {Ly:.1f} μm', fontsize=16)
...
def train_PINN(D11, D12, D21, D22, Lx, Ly, T_max, epochs=2000, lr=0.001, output_dir="pinn_solutions"):
    ...
    plot_losses(loss_history, Ly, output_dir)
