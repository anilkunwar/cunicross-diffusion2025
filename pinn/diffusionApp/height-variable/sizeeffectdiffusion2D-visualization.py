...
def load_solutions(solution_dir):
    ...
    return solutions, lys, load_logs
...
def attention_weighted_interpolation(solutions, lys, ly_target, sigma=2.5):
...
@st.cache_data
def load_and_interpolate_solution(solutions, lys, ly_target, tolerance=1e-4):
    """Load or interpolate solution for target Ly."""
    lys = np.array(lys)
    exact_match_indices = np.where(np.abs(lys - ly_target) < tolerance)[0]
    ...
    if exact_match_indices.size > 0:
        ...
        return attention_weighted_interpolation(solutions, lys, ly_target, sigma=2.5)

@st.cache_data
def compute_center_concentrations(solutions, lys):
    ...
    for sol, ly in zip(solutions, lys):
        center_y = ly / 2
        times = sol['times']
        c1_center = []
        c2_center = []
...
def plot_center_concentrations(center_concentrations, selected_lys, solution, ly_target, is_interpolated):
    ...
    if is_interpolated:
        ...
        ax1.plot(solution['times'], c1_center, label=f'ML Predicted (Ly = {ly_target:.1f} μm)',
                 linewidth=2.5, color='black', linestyle='--')
    
    ax1.set_xlabel('Time (s)', fontsize=14)
    ax1.set_ylabel('Cu Concentration', fontsize=14)
    ax1.set_title(f'Cu Concentration at Center (x={solution["params"]["Lx"]/2:.1f} μm, y=Ly/2) vs. Time',
                  fontsize=16)
    ax1.grid(True, which="both", ls="--", alpha=0.7)
    ax1.legend(fontsize=12, loc='best', bbox_to_anchor=(1.05, 1), borderaxespad=0.)
    ...
    if is_interpolated:
        ...
        ax2.plot(solution['times'], c2_center, label=f'ML Predicted (Ly = {ly_target:.1f} μm)',
                 linewidth=2.5, color='black', linestyle='--')
    
    ax2.set_xlabel('Time (s)', fontsize=14)
    ax2.set_ylabel('Ni Concentration', fontsize=14)
    ax2.set_title(f'Ni Concentration at Center (x={solution["params"]["Lx"]/2:.1f} μm, y=Ly/2) vs. Time',
                  fontsize=16)
    ax2.grid(True, which="both", ls="--", alpha=0.7)
    ax2.legend(fontsize=12, loc='best', bbox_to_anchor=(1.05, 1), borderaxespad=0.)
...
def plot_solution(solution, time_index, downsample):
    ...
    fig.add_trace(go.Heatmap(
        ...
        colorscale='Viridis',
        showscale=True,
        colorbar=dict(title='Cu Conc', x=0.45),
        zsmooth='best'
    ), row=1, col=1)
    ...
    fig.add_trace(go.Heatmap(
        ...
        colorscale='Magma',
        showscale=True,
        colorbar=dict(title='Ni Conc', x=1.02),
        zsmooth='best'
    ), row=1, col=2)
...
def main():
    ...
    try:
        solution = load_and_interpolate_solution(solutions, lys, ly_target)
    except Exception as e:
    ...
    st.info("Note: Training loss plots are available in the pinn_solutions/ directory (e.g., loss_plot_ly_XX.X.png).")
    ...
    if show_center_plots:
        ...
        center_concentrations = compute_center_concentrations(solutions, lys)
        available_lys = [conc['Ly'] for conc in center_concentrations]
        selected_lys = st.multiselect(
            "Select Ly values to plot",
            options=available_lys,
            default=available_lys,
            format_func=lambda x: f"{x:.1f} μm"
        )
        
        is_interpolated = solution.get('interpolated', False)
        fig1, fig2 = plot_center_concentrations(
            center_concentrations, selected_lys, solution, ly_target, is_interpolated
        )
        st.pyplot(fig1)
        st.pyplot(fig2)
        plt.close(fig1)
