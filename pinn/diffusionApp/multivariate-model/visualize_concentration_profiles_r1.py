...
def attention_weighted_interpolation(solutions, params_list, ly_target, c_cu_target, c_ni_target, sigma_ly=0.2, sigma_ccu=0.2, sigma_cni=0.2):
    ...
    # Normalize parameters for all solutions
    ...
    # Compute distances for all solutions
...
def plot_2d_concentration(solution, time_index, output_dir="figures"):
    ...
    c1 = solution['c1_preds'][time_index]  # rows=y, cols=x
    ...
    # Cu heatmap
    im1 = ax1.imshow(
        c1,  # No transpose: rows=y (vertical), cols=x (horizontal)
        origin='lower',
        extent=[0, Lx, 0, Ly],
        cmap='viridis',
        vmin=0,
...
def plot_centerline_curves(solution, time_indices, output_dir="figures"):
    x_coords = solution['X'][:, 0]
    y_coords = solution['Y'][0, :]
    Lx = solution['params']['Lx']
    Ly = solution['params']['Ly']
    ...
    for idx, t_idx in enumerate(time_indices):
        ...
        c1 = solution['c1_preds'][t_idx][:, center_idx]  # Cu along x = Lx/2
        c2 = solution['c2_preds'][t_idx][:, center_idx]  # Ni along x = Lx/2
    ...
    ax1.set_title(f'Cu Concentration at x = {Lx/2:.1f} μm')
    ...
    ax2.set_title(f'Ni Concentration at x = {Lx/2:.1f} μm')
...
def plot_parameter_sweep(solutions, params_list, selected_params, time_index, output_dir="figures"):
    Lx = solutions[0]['params']['Lx']
    center_idx = 25  # x = Lx/2
    ...
    ax1.set_xlabel('y (μm)')
    ax1.set_ylabel('Cu Conc. (mol/cc)')
    ax1.set_title(f'Cu Concentration at x = {Lx/2:.1f} μm, t = {solutions[0]["times"][time_index]:.1f} s')
    ax1.legend(fontsize=8, loc='upper right')
    ax1.grid(True, linestyle='--', alpha=0.5)
    ax1.ticklabel_format(style='sci', axis='y', scilimits=(0,0))
    ...
    ax2.set_xlabel('y (μm)')
    ax2.set_ylabel('Ni Conc. (mol/cc)')
    ax2.set_title(f'Ni Concentration at x = {Lx/2:.1f} μm, t = {solutions[0]["times"][time_index]:.1f} s')
    ax2.legend(fontsize=8, loc='upper right')
    ax2.grid(True, linestyle='--', alpha=0.5)
    ax2.ticklabel_format(style='sci', axis='y', scilimits=(0,0))
    ...
    os.makedirs(output_dir, exist_ok=True)
    base_filename = f"conc_sweep_t_{solutions[0]['times'][time_index]:.1f}"
    plt.savefig(os.path.join(output_dir, f"{base_filename}.png"), dpi=300, bbox_inches='tight')
    plt.savefig(os.path.join(output_dir, f"{base_filename}.pdf"), bbox_inches='tight')
    plt.close()
...
def main():
    ...
    fig_2d, filename_2d = plot_2d_concentration(solution, time_index)
    st.pyplot(fig_2d)
    st.download_button(
    ...
    if time_indices:
        fig_curves, filename_curves = plot_centerline_curves(solution, time_indices)
    ...
    if selected_params:
        fig_sweep, filename_sweep = plot_parameter_sweep(solutions, params_list, selected_params, sweep_time_index)
        st.pyplot(fig_sweep)
        st.download_button(
