...
def plot_uphill_heatmap_matplotlib(solution, time_index, cmap='viridis', vmin=None, vmax=None,
                                   figsize=(10,4.5), colorbar=True, cbar_label='J·∇c',
    ...
    if cbar_position == 'top':
        ...
        gs = gridspec.GridSpec(2, 2, figure=fig, 
                              ...
                              hspace=0.9,  # ✅ INCREASED: More space for suptitle
                              wspace=0.3)
...
def main():
    ...
    st.sidebar.markdown("---")
    st.sidebar.subheader("🎨 Colorbar Options")
    cbar_position = st.sidebar.selectbox(
        "📊 Colorbar Position", 
        ['top', 'right', 'left'], 
        index=0,  # Default: TOP (publication favorite!)
        help="Top: Horizontal (saves space)\nRight: Traditional\nLeft: Flipped layout"
    )
    cbar_size_slider = st.sidebar.slider(
        "📏 Colorbar Size", 
        0.08, 0.25, 0.15, 0.01,
        help="Larger = thicker colorbar (0.08-0.25)"
    )
    ...
    # ✅ NEW: Pass colorbar position and size!
    fig_hm, max_pos_cu, max_pos_ni, frac_cu, frac_ni = plot_uphill_heatmap_matplotlib(
        solution, time_index,
        cmap=cmap_choice, vmin=vmin_val, vmax=vmax_val,
        figsize=(fig_width, fig_height),
        downsample=downsample,
        cbar_position=cbar_position,  # 🎛️ From sidebar
        cbar_size=cbar_size_slider    # 🎛️ From sidebar
    )
    st.pyplot(fig_hm)
