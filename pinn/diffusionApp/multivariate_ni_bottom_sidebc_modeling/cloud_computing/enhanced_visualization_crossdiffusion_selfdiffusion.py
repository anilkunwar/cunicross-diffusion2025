...
def plot_solution(solution, time_index, downsample, title_suffix="", cu_colormap='viridis', ni_colormap='magma', font_size=12, tick_interval=10, grid_thickness=0.5, border_thickness=1):
    ...
    fig.update_xaxes(title_text="x (μm)", range=[0, Lx], gridcolor='white', zeroline=False, row=1, col=1, dtick=tick_interval)
    fig.update_yaxes(title_text="y (μm)", range=[0, Ly], gridcolor='white', zeroline=False, row=1, col=1, dtick=tick_interval)
    fig.update_xaxes(title_text="x (μm)", range=[0, Lx], gridcolor='white', zeroline=False, row=1, col=2, dtick=tick_interval)
    fig.update_yaxes(title_text="y (μm)", range=[0, Ly], gridcolor='white', zeroline=False, row=1, col=2, dtick=tick_interval)

    st.plotly_chart(fig, use_container_width=True)

def plot_flux_comparison(solutions, diff_type, ly_values, time_index, downsample, font_size=12, tick_interval=10, grid_thickness=0.5, border_thickness=1, arrow_thickness=1):
    ...
    # axis formatting for all subplots, with per-column y-range for visible height difference
    total_rows = 3
    total_cols = 4
    for row in range(1, total_rows + 1):
        for col in [1, 2]:
            fig.update_xaxes(title_text="x (μm)", range=[0, sol1['params']['Lx']], gridcolor='white', zeroline=False, row=row, col=col, dtick=tick_interval)
            fig.update_yaxes(title_text="y (μm)", range=[0, ly_values[0]], gridcolor='white', zeroline=False, row=row, col=col, dtick=tick_interval)
        for col in [3, 4]:
            fig.update_xaxes(title_text="x (μm)", range=[0, sol1['params']['Lx']], gridcolor='white', zeroline=False, row=row, col=col, dtick=tick_interval)
            fig.update_yaxes(title_text="y (μm)", range=[0, ly_values[1]], gridcolor='white', zeroline=False, row=row, col=col, dtick=tick_interval)

    st.plotly_chart(fig, use_container_width=True)

def plot_line_comparison(solutions, diff_type, ly_values, time_index, line_thickness=2, label_font_size=12, tick_font_size=10, tick_interval=10, spine_thickness=1.5, color_ly1='#1f77b4', color_ly2='#ff7f0e'):
    ...
    for idx, (sol, Ly, color, linestyle) in enumerate(zip([sol1, sol2], ly_values, colors, ['-', '--'])):
        ...
        ax1.plot(c1_center, y_coords, label=f'Ly = {Ly:.1f} μm', linestyle=linestyle, linewidth=line_thickness, color=color)
        ax2.plot(c2_center, y_coords, label=f'Ly = {Ly:.1f} μm', linestyle=linestyle, linewidth=line_thickness, color=color)

    ax1.set_xlabel('Cu Concentration (mol/cm³)', fontsize=label_font_size)
    ax1.set_ylabel('y (μm)', fontsize=label_font_size)
    ...
    ax1.set_yticks(np.arange(0, max(ly_values) + tick_interval, tick_interval))
    for spine in ax1.spines.values():
        spine.set_linewidth(spine_thickness)

    ax2.set_xlabel('Ni Concentration (mol/cm³)', fontsize=label_font_size)
    ax2.set_ylabel('y (μm)', fontsize=label_font_size)
    ax2.set_title(f'Ni @ x=center, t={t_val:.1f}s', fontsize=label_font_size + 2)
    ax2.legend(fontsize=label_font_size - 2)
    ax2.tick_params(labelsize=tick_font_size)
    ax2.set_yticks(np.arange(0, max(ly_values) + tick_interval, tick_interval))
...
def plot_center_concentrations(center_concentrations, diff_type, line_thickness=2, label_font_size=12, tick_font_size=10, tick_interval=50, spine_thickness=1.5, color_ly1='#1f77b4', color_ly2='#ff7f0e'):
    ...
    ax1.set_ylabel('Cu Concentration (mol/cm³)', fontsize=label_font_size)
    ax1.set_title('Cu Concentration at Center', fontsize=label_font_size + 2)
    ax1.legend(fontsize=label_font_size - 2)
    ax1.tick_params(labelsize=tick_font_size)
    ax1.set_xticks(np.arange(0, max(conc['times']) + tick_interval, tick_interval))
    for spine in ax1.spines.values():
        spine.set_linewidth(spine_thickness)

    ax2.set_xlabel('Time (s)', fontsize=label_font_size)
    ax2.set_ylabel('Ni Concentration (mol/cm³)', fontsize=label_font_size)
    ax2.set_title('Ni Concentration at Center', fontsize=label_font_size + 2)
    ax2.legend(fontsize=label_font_size - 2)
    ax2.tick_params(labelsize=tick_font_size)
    ax2.set_xticks(np.arange(0, max(conc['times']) + tick_interval, tick_interval))
    for spine in ax2.spines.values():
        spine.set_linewidth(spine_thickness)

    ax3.set_xlabel('Time (s)', fontsize=label_font_size)
    ax3.set_ylabel('Cu Flux Magnitude', fontsize=label_font_size)
    ax3.set_title('Cu Flux Magnitude at Center', fontsize=label_font_size + 2)
    ax3.legend(fontsize=label_font_size - 2)
    ax3.tick_params(labelsize=tick_font_size)
    ax3.set_xticks(np.arange(0, max(conc['times']) + tick_interval, tick_interval))
    if any(np.any(conc['J1_mag_center'] > 0) for conc in center_concentrations):
        ax3.set_yscale('log')
    for spine in ax3.spines.values():
        spine.set_linewidth(spine_thickness)

    ax4.set_xlabel('Time (s)', fontsize=label_font_size)
    ax4.set_ylabel('Ni Flux Magnitude', fontsize=label_font_size)
    ax4.set_title('Ni Flux Magnitude at Center', fontsize=label_font_size + 2)
    ax4.legend(fontsize=label_font_size - 2)
    ax4.tick_params(labelsize=tick_font_size)
    ax4.set_xticks(np.arange(0, max(conc['times']) + tick_interval, tick_interval))
...
def main():
    ...
    tick_interval = st.sidebar.slider("Tick Interval (μm/s)", 5, 50, 10)
    ...
    with tab1:
        ...
        for ly in ly_values:
            ...
            if solution:
                plot_solution(solution, time_index, downsample, title_suffix=f"[{diff_type.replace('_', ' ')}, Ly={ly:.1f}]", cu_colormap=cu_colormap, ni_colormap=ni_colormap, font_size=font_size, tick_interval=tick_interval, grid_thickness=grid_thickness, border_thickness=border_thickness)
            else:
                st.error(f"No solution for {diff_type}, Ly={ly:.1f}")

    with tab2:
        st.subheader("Flux Fields Comparison")
        plot_flux_comparison(solutions, diff_type, ly_values, time_index, downsample, font_size=font_size, tick_interval=tick_interval, grid_thickness=grid_thickness, border_thickness=border_thickness, arrow_thickness=arrow_thickness)

    with tab3:
        st.subheader("Central Line Profiles Comparison")
        plot_line_comparison(solutions, diff_type, ly_values, time_index, line_thickness=line_thickness, label_font_size=label_font_size, tick_font_size=tick_font_size, tick_interval=tick_interval, spine_thickness=spine_thickness, color_ly1=color_ly1, color_ly2=color_ly2)
    ...
    with tab4:
        ...
        if center_concentrations:
            plot_center_concentrations(center_concentrations, diff_type, line_thickness=line_thickness, label_font_size=label_font_size, tick_font_size=tick_font_size, tick_interval=tick_interval, spine_thickness=spine_thickness, color_ly1=color_ly1, color_ly2=color_ly2)
        else:
