...
np.random.seed(42)
# Helper function for cache key and file naming
def get_cache_key(Ly, C_Ni_bottom):
    key_string = f"Ly_{Ly:.1f}_C_Ni_bottom_{C_Ni_bottom:.2e}"
...
class DualScaledPINN(nn.Module):
    def __init__(self, D11, D12, D21, D22, Lx, Ly, T_max, C_Ni_bottom):
        super().__init__()
        self.D11 = D11
        self.D12 = D12
        self.D21 = D21
        ...
        self.C_Cu_norm = 0.0 # Since Cu is fixed at 0
        self.C_Ni_norm = (C_Ni_bottom - 4.0e-4) / (1.8e-3 - 4.0e-4)
        ...
        self.cu_head[2].weight.data.fill_(0.0)
        self.ni_head[2].weight.data.fill_(C_Ni_bottom)
...
def boundary_loss_bottom(model):
    ...
    return (torch.mean((c_pred[:, 0] - C_CU_BOTTOM)**2) +
            torch.mean((c_pred[:, 1] - model.C_Ni_bottom)**2))
...
def validate_boundary_conditions(solution, tolerance=1e-6):
    ...
    if abs(top_ni_mean - C_NI_TOP) > tolerance: # Fixed from original
    ...
    if abs(bottom_ni_mean - solution['params']['C_Ni_bottom']) > tolerance: # Dynamic C_Ni_bottom
        results['bottom_bc_ni'] = False
        results['details'].append(f"Bottom Ni: {bottom_ni_mean:.2e} != {solution['params']['C_Ni_bottom']:.2e}")
...
def plot_2d_profiles(solution, time_idx, output_dir, file_suffix):
    ...
    im2 = plt.imshow(solution['c2_preds'][time_idx], origin='lower',
                     ...
                     vmin=0, vmax=solution['params']['C_Ni_bottom'])
...
def train_model(D11, D12, D21, D22, Lx, Ly, T_max, C_Ni_bottom, epochs, lr, output_dir, _hash):
    os.makedirs(output_dir, exist_ok=True)
    logger.info(f"Starting training with Ly={Ly}, C_Ni_bottom={C_Ni_bottom}, epochs={epochs}, lr={lr}")
    model = DualScaledPINN(D11, D12, D21, D22, Lx, Ly, T_max, C_Ni_bottom)
...
def generate_and_save_solution(_model, times, param_set, output_dir, file_suffix):
    ...
    solution_filename = os.path.join(output_dir,
        f"solution_ni_selfdiffusion_{file_suffix}_tmax_{param_set['t_max']:.1f}.pkl")
...
def create_zip_file(_files, output_dir, file_suffix):
    ...
    try:
        ...
        zip_filename = os.path.join(output_dir, f'pinn_solutions_ni_selfdiffusion_{file_suffix}.zip')
...
def get_file_bytes(file_path):
    ...
    return None
@st.cache_data(ttl=3600, show_spinner=False)
def train_and_generate_solution(_model, loss_history, output_dir, file_suffix, Ly, C_Ni_bottom):
    os.makedirs(output_dir, exist_ok=True)
    ...
    param_set = {
        ...
        'C_Cu_top': C_CU_TOP, 'C_Ni_bottom': C_Ni_bottom, # Dynamic
        'epochs': epochs
    }
...
def main():
    ...
    # User inputs
    Ly = st.sidebar.number_input("Domain height Ly (μm)", min_value=10.0, max_value=200.0, value=50.0, step=10.0)
    C_Ni_bottom = st.sidebar.number_input("Ni concentration at bottom (mol/cc)", min_value=1e-6, max_value=2e-3, value=4.0e-4, format="%.2e")
   
    _hash, file_suffix = get_cache_key(Ly, C_Ni_bottom)
        ...
        try:
            ...
            with st.spinner("Running simulation..."):
                model, loss_history = train_model(
                    D11, D12, D21, D22, Lx, Ly, T_max, C_Ni_bottom, epochs, lr, OUTPUT_DIR, current_hash
                )
                ...
                solution, file_info = train_and_generate_solution(
                    model, loss_history, OUTPUT_DIR, file_suffix, Ly, C_Ni_bottom
                )
