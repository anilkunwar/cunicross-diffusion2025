...
def boundary_loss_bottom(model):
    ...
    c_pred = model(x, y, t)
    return torch.mean((c_pred - model.C_Ni_bottom)**2)
...
def boundary_loss_top(model):
    ...
    c_pred = model(x, y, t)
    return torch.mean((c_pred - C_NI_TOP)**2)
...
def validate_boundary_conditions(solution, tolerance=1e-6):
    ...
    if abs(bottom_mean - solution['params']['C_Ni_bottom']) > tolerance:
        results['bottom_bc'] = False
        results['details'].append(f"Bottom: {bottom_mean:.2e} != {solution['params']['C_Ni_bottom']:.2e}")
...
def plot_losses(loss_history, output_dir, _hash, Ly, C_Ni_bottom):
...
def plot_2d_profiles(solution, time_idx, output_dir, _hash):
    ...
    C_Ni_bottom = solution['params']['C_Ni_bottom']
...
@st.cache_data(ttl=3600, show_spinner=False)
def create_zip_file(_files, output_dir, _hash, Ly, C_Ni_bottom):
    os.makedirs(output_dir, exist_ok=True)
...
@st.cache_data(ttl=3600, show_spinner=False)
def train_and_generate_solution(_model, loss_history, output_dir, _hash_key, Ly, C_Ni_bottom):
    os.makedirs(output_dir, exist_ok=True)
    ...
    loss_plot_filename = plot_losses(loss_history, output_dir, _hash_key, Ly, C_Ni_bottom)
...
def main():
    ...
    c2 = st.number_input("Bottom Ni concentration c2 (mol/cc)", min_value=1.0e-4, max_value=6.0e-4, value=1.0e-4, step=1e-5, format="%.1e")
    
    initialize_session_state()
    current_hash = get_cache_key(Ly, c2, epochs, lr, D22)
        ...
        try:
            with st.spinner("Running simulation..."):
                model, loss_history = train_model(
                    D22, Lx, Ly, T_max, c2, epochs, lr, OUTPUT_DIR, current_hash
                )
                ...
                solution, file_info = train_and_generate_solution(
                    model, loss_history, OUTPUT_DIR, current_hash, Ly, c2
                )
    ...
    if solution and file_info:
        ...
        if st.button("Generate ZIP File"):
            with st.spinner("Creating ZIP file..."):
                ...
                zip_filename = create_zip_file(files_to_zip, OUTPUT_DIR, (Ly,), Ly, c2)
