...
# === PyTorch Model with Fixed Tensor to NumPy Conversion ===
...
class MultiParamAttentionInterpolator(nn.Module):
    ...
    def compute_weights(self, params_list, ly_target, c_cu_target, c_ni_target):
        ...
        # Fixed tensor to numpy conversion to avoid FLAN-T5 error
        return {
            'W_q': self.W_q.weight.detach().cpu().float().numpy(),
            'W_k': self.W_k.weight.detach().cpu().float().numpy(),
            'attention_weights': attn_weights.detach().cpu().numpy(),
            'spatial_weights': spatial_weights.detach().cpu().numpy(),
            'combined_weights': combined.detach().cpu().numpy(),
            'norm_sources': norm_sources,
            'norm_target': norm_target
        }

# === Diffusion Calculation Class ===
...
with st.sidebar:
    ...
    substrate_type = st.selectbox("Substrate",
                                 ["Cu(top)-Ni(bottom) Asymmetric",
                                  "Cu/Sn2.5Ag/Cu Symmetric",
                                  "Ni/Sn2.5Ag/Ni Symmetric"])
