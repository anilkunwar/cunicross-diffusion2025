...
# ----------------------------------------------------------------------
# Matplotlib style (exactly the one you love)
# ----------------------------------------------------------------------
mpl.rcParams.update({
...
# ----------------------------------------------------------------------
# 4. Build the (50 × 8) matrices for the sunburst
# ----------------------------------------------------------------------
LY_SPOKES = [30, 40, 50, 60, 70, 80, 90, 100]   # µm
N_TIME    = 50

def build_sunburst_matrices(solutions, params_list, interpolator,
                           c_cu_target, c_ni_target, ly_fraction):
...
# ----------------------------------------------------------------------
# 6. Radar chart (snapshot at one time)
# ----------------------------------------------------------------------
...
def main():
    ...
    st.markdown("""
    ### Physical Meaning
    - **Spokes (Angular Axis):** Different *film thicknesses* (`Ly`) — each spoke = one `Ly` value.  
    - **Radial Length (Radius):** *Time evolution* of the centre-point concentration.  
    - **Sunburst** = concentration vs. time (radial) for many `Ly` (angular).  
    - **Radar** = snapshot at a chosen time, comparing the 8 `Ly` values.
    """)
    ...
    # centre-fraction selector (Ly/2, Ly/3, …)
    frac_options = {"Ly/2":0.5, "Ly/3":1/3, "Ly/4":0.25}
    ...
    # ---- build matrices (cached automatically) ----
    ...
    with col1:
        fig_cu, png_cu, pdf_cu = plot_sunburst(
            cu_mat, "Cu centre concentration", cmap,
            0, c_cu_target, log_scale, ly_dir, "sunburst_cu"
        )
        st.pyplot(fig_cu)
        with open(png_cu,"rb") as f:
            st.download_button("Cu PNG", f, "sunburst_cu.png", "image/png")
        with open(pdf_cu,"rb") as f:
            st.download_button("Cu PDF", f, "sunburst_cu.pdf", "application/pdf")

    with col2:
        fig_ni, png_ni, pdf_ni = plot_sunburst(
            ni_mat, "Ni centre concentration", cmap,
            0, c_ni_target, log_scale, ly_dir, "sunburst_ni"
        )
        st.pyplot(fig_ni)
        with open(png_ni,"rb") as f:
            st.download_button("Ni PNG", f, "sunburst_ni.png", "image/png")
        with open(pdf_ni,"rb") as f:
            st.download_button("Ni PDF", f, "sunburst_ni.pdf", "application/pdf")
    ...
    st.pyplot(fig_r)
    with open(png_r,"rb") as f:
        st.download_button("Radar PNG", f, f"radar_t{t_val:.0f}.png", "image/png")
    with open(pdf_r,"rb") as f:
        st.download_button("Radar PDF", f, f"radar_t{t_val:.0f}.pdf", "application/pdf")
