...
def main():
    ...
    if "Pure Cu" in mode:
        c_ni_target = 0.0
        st.sidebar.warning("Ni boundary forced to 0 (air/no-flux at top)")
    elif "Pure Ni" in mode:
        c_cu_target = 0.0
        st.sidebar.warning("Cu boundary forced to 0 (air/no-flux at top)")
    ...
    if selected_session == "Create New Session":
        ...
        # ---- build matrices with time scaling (with special modes) ----
        with st.spinner("Computing sunburst matrices..."):
            if mode == "Pure Cu Diffusion (Cu substrate bottom, air top)":
                # Force Ni boundary = 0 → no Ni diffusion
                cu_mat, ni_mat, times = build_sunburst_matrices(
                    sols, params, interpolator,
                    c_cu_target, 0.0, ly_fraction, LY_SPOKES, time_log_scale
                )
                st.success("Pure Cu mode: Ni boundary set to 0 (no-flux top)")
                
            elif mode == "Pure Ni Diffusion (Ni substrate bottom, air top)":
                # Force Cu boundary = 0 → no Cu diffusion
                cu_mat, ni_mat, times = build_sunburst_matrices(
                    sols, params, interpolator,
                    0.0, c_ni_target, ly_fraction, LY_SPOKES, time_log_scale
                )
                st.success("Pure Ni mode: Cu boundary set to 0 (no-flux top)")
                
            else:  # Standard coupled mode
                cu_mat, ni_mat, times = build_sunburst_matrices(
                    sols, params, interpolator,
