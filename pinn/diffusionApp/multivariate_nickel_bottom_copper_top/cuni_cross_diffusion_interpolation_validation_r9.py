...
=====================================================================
PRODUCTION-READY VERSION: Advanced features + deployment support
- ✅ All previous fixes + robustness improvements
- ✅ Export plots as PNG/SVG/PDF via Plotly + Kaleido
- ✅ Save/load customization presets (JSON-based)
- ✅ Real-time preview (Streamlit auto-rerun on widget changes)
- ✅ Additional chart types: Box plots, Parameter-space heatmaps
...
def plot_metrics_boxplot(metrics_list: List[ValidationMetrics],
    ...
    for metrics in metrics_list:
        for metric_name in metric_names:
            attr_name = ALL_VISUAL_METRICS.get(metric_name)
                ...
                if metric_name in ['MSE (Cu)', 'MSE (Ni)', 'MAE (Cu)', 'MAE (Ni)', 
                    ...
                    scale = {'mse': 1e-6, 'mae': 1e-4, 'max_error': 1e-4, 
                        ...
                        {'MSE (Cu)': 'mse', 'MSE (Ni)': 'mse', 'MAE (Cu)': 'mae', 'MAE (Ni)': 'mae',
                         'Max Error (Cu)': 'max_error', 'Max Error (Ni)': 'max_error',
                         'PDE Residual (Mean)': 'pde_residual_mean', 'PDE Residual (Max)': 'pde_residual_max',
                         'BC Error Top (Cu)': 'bc_error', 'BC Error Top (Ni)': 'bc_error',
                         'BC Error Bottom (Cu)': 'bc_error', 'BC Error Bottom (Ni)': 'bc_error',
                         'Mass Conservation': 'mass_error', 'Param. Distance': 'param_distance'}.get(metric_name, 'mse'),
                        1e-4)
                    val = max(0.0, min(1.0, np.exp(-val / scale)))
                elif metric_name in ['Mass Conservation']:
                    val = max(0.0, min(1.0, 1.0 - val))
                # R², SSIM, Overall Score are already 0-1
    ...
    if df.empty:
        fig = go.Figure()
        fig.add_annotation(
            text="⚠️ No data available for box plot",
            xref="paper", yref="paper",
            x=0.5, y=0.5,
            showarrow=False,
            font=dict(size=14, color="gray"),
...
def plot_parameter_space_heatmap(solutions: List[Dict],
                                metric_name: str,
                                x_param: str = 'Ly',
                                y_param: str = 'C_Cu',
    ...
    for sol in solutions:
        ...
        if not metrics or not hasattr(metrics, 'to_dict'):
            continue
        ...
        # Get metric value
        attr_name = ALL_VISUAL_METRICS.get(metric_name)
        if attr_name and attr_name in metrics_dict:
            ...
            if metric_name in ['MSE (Cu)', 'MSE (Ni)', 'MAE (Cu)', 'MAE (Ni)', 
                ...
                scale = {'mse': 1e-6, 'mae': 1e-4, 'max_error': 1e-4, 
                    ...
                    {'MSE (Cu)': 'mse', 'MSE (Ni)': 'mse', 'MAE (Cu)': 'mae', 'MAE (Ni)': 'mae',
                     ...
                     'Mass Conservation': 'mass_error', 'Param. Distance': 'param_distance'}.get(metric_name, 'mse'),
                    1e-4)
                z_val = max(0.0, min(1.0, np.exp(-z_val / scale)))
            elif metric_name in ['Mass Conservation']:
...
def export_plot_to_image(fig: go.Figure, format: str = 'png', width: int = 1200, height: int = 800, scale: int = 2) -> bytes:
    """Export Plotly figure to image bytes (PNG/SVG/PDF)."""
    try:
        # Try to use kaleido for high-quality export
        import kaleido
        img_bytes = fig.to_image(format=format, width=width, height=height, scale=scale)
        return img_bytes
    except ImportError:
        # Fallback: use plotly's built-in export (lower quality, no PDF)
        if format == 'svg':
            return fig.to_image(format='svg', width=width, height=height).encode('utf-8')
        else:
            # For PNG, return base64-encoded PNG as fallback
        ...
        logger.warning(f"Export failed: {e}. Returning placeholder.")
        # Return minimal placeholder image
        from PIL import Image, ImageDraw
        img = Image.new('RGB', (width, height), color='white')
        draw = ImageDraw.Draw(img)
        draw.text((width//2-100, height//2), f"Export failed: {str(e)[:50]}", fill='red')
        buf = io.BytesIO()
        img.save(buf, format='PNG')
        return buf.getvalue()


def save_customization_preset(preset_name: str, customization: Dict, metrics_config: Dict) -> str:
    """Save current customization settings to a JSON preset file."""
    preset = {
        'name': preset_name,
    ...
    filepath = os.path.join(PRESETS_DIR, f"{preset_name.replace(' ', '_')}.json")
    with open(filepath, 'w') as f:
...
def load_customization_preset(preset_name: str) -> Optional[Dict]:
    """Load customization settings from a preset file."""
    filepath = os.path.join(PRESETS_DIR, f"{preset_name.replace(' ', '_')}.json")
...
def list_available_presets() -> List[str]:
    """List all available customization preset names."""
    presets = []
    for filename in os.listdir(PRESETS_DIR):
        if filename.endswith('.json'):
            preset_name = filename[:-5].replace('_', ' ')
...
def inject_dark_mode_css():
    """Inject CSS for dark mode theming."""
    st.markdown("""
    <style>
    /* Dark mode base */
    .stApp {
...
def main():
    ...
    with st.sidebar:
        ...
        with st.expander("💾 Save/Load Presets", expanded=False):
            preset_name = st.text_input("Preset name", placeholder="e.g., Publication Ready")
            ...
            with col_save:
                if st.button("💾 Save Preset", width='stretch'):
                    if preset_name:
                        metrics_config = {
                            'bar_metrics': st.session_state.get('selected_bar_metrics', DEFAULT_BAR_METRICS),
                            'radar_metrics': st.session_state.get('selected_radar_metrics', DEFAULT_RADAR_METRICS)
                        }
                        filepath = save_customization_preset(
                            preset_name,
            ...
            with col_load:
                ...
                if available_presets:
                    selected_preset = st.selectbox("Load preset", available_presets)
                    if st.button("📥 Load", width='stretch'):
                        preset = load_customization_preset(selected_preset)
                        if preset:
                            st.session_state.plot_customization.update(preset['customization'])
                            if 'metrics' in preset:
                                if 'bar_metrics' in preset['metrics']:
                                    st.session_state.selected_bar_metrics = preset['metrics']['bar_metrics']
                                if 'radar_metrics' in preset['metrics']:
                                    st.session_state.selected_radar_metrics = preset['metrics']['radar_metrics']
            ...
            if st.button("🗑️ Delete Preset", width='stretch'):
                available_presets = list_available_presets()
                if available_presets:
                    preset_to_delete = st.selectbox("Select preset to delete", available_presets)
                    if st.button(f"Delete {preset_to_delete}", width='stretch'):
                        filepath = os.path.join(PRESETS_DIR, f"{preset_to_delete.replace(' ', '_')}.json")
                        if os.path.exists(filepath):
                            os.remove(filepath)
                            st.success(f"🗑️ Deleted preset: {preset_to_delete}")
                            if st.session_state.current_preset == preset_to_delete:
    ...
    with tab1:
        ...
        with col_export1:
            if st.button("📥 Export PNG", key="export_png_tab1"):
                img_bytes = export_plot_to_image(fig_bar, format='png')
                st.download_button(
        ...
        with col_export2:
            if st.button("📥 Export SVG", key="export_svg_tab1"):
                img_bytes = export_plot_to_image(fig_bar, format='svg')
                st.download_button(
                    label="⬇️ Download SVG",
                    data=img_bytes,
                    file_name=f"{chart_title.replace(' ', '_')}.svg",
                    mime="image/svg+xml"
                )
        with col_export3:
            if st.button("📥 Export PDF", key="export_pdf_tab1"):
                img_bytes = export_plot_to_image(fig_bar, format='pdf')
                st.download_button(
    ...
    with tab2:
        ...
        if selected_case_labels:
            ...
            col_export1, col_export2, col_export3 = st.columns(3)
            with col_export1:
                if st.button("📥 Export PNG", key="export_png_tab2_bar"):
                    img_bytes = export_plot_to_image(fig_multi_bar, format='png')
                    st.download_button(
                        label="⬇️ Download PNG",
                        data=img_bytes,
                        file_name="multi_case_bar_chart.png",
                        mime="image/png"
                    )
            with col_export2:
                if st.button("📥 Export SVG", key="export_svg_tab2_bar"):
                    img_bytes = export_plot_to_image(fig_multi_bar, format='svg')
                    st.download_button(
                        label="⬇️ Download SVG",
                        data=img_bytes,
                        file_name="multi_case_bar_chart.svg",
                        mime="image/svg+xml"
                    )
            with col_export3:
                if st.button("📥 Export PDF", key="export_pdf_tab2_bar"):
                    img_bytes = export_plot_to_image(fig_multi_bar, format='pdf')
                    st.download_button(
                        label="⬇️ Download PDF",
                        data=img_bytes,
                        file_name="multi_case_bar_chart.pdf",
                        mime="application/pdf"
                    )
            ...
            col_export1, col_export2, col_export3 = st.columns(3)
            with col_export1:
                if st.button("📥 Export PNG", key="export_png_tab2_radar"):
                    img_bytes = export_plot_to_image(fig_multi_radar, format='png')
                    st.download_button(
                        label="⬇️ Download PNG",
                        data=img_bytes,
                        file_name="multi_case_radar_chart.png",
                        mime="image/png"
                    )
            with col_export2:
                if st.button("📥 Export SVG", key="export_svg_tab2_radar"):
                    img_bytes = export_plot_to_image(fig_multi_radar, format='svg')
                    st.download_button(
                        label="⬇️ Download SVG",
                        data=img_bytes,
                        file_name="multi_case_radar_chart.svg",
                        mime="image/svg+xml"
                    )
            with col_export3:
                if st.button("📥 Export PDF", key="export_pdf_tab2_radar"):
                    img_bytes = export_plot_to_image(fig_multi_radar, format='pdf')
                    st.download_button(
                        label="⬇️ Download PDF",
                        data=img_bytes,
                        file_name="multi_case_radar_chart.pdf",
                        mime="application/pdf"
                    )
    ...
    with tab3:
        st.subheader("📦 Metric Distribution (Box Plots)")
        
        available_metrics = list(ALL_VISUAL_METRICS.keys())
        selected_box_metrics = st.multiselect(
        ...
        if selected_box_metrics and results:
            ...
            col_export1, col_export2, col_export3 = st.columns(3)
            with col_export1:
                if st.button("📥 Export PNG", key="export_png_tab3"):
                    img_bytes = export_plot_to_image(fig_box, format='png')
                    st.download_button(
                        label="⬇️ Download PNG",
                        data=img_bytes,
                        file_name="metrics_boxplot.png",
                        mime="image/png"
                    )
            with col_export2:
                if st.button("📥 Export SVG", key="export_svg_tab3"):
                    img_bytes = export_plot_to_image(fig_box, format='svg')
                    st.download_button(
                        label="⬇️ Download SVG",
                        data=img_bytes,
                        file_name="metrics_boxplot.svg",
                        mime="image/svg+xml"
                    )
            with col_export3:
                if st.button("📥 Export PDF", key="export_pdf_tab3"):
                    img_bytes = export_plot_to_image(fig_box, format='pdf')
                    st.download_button(
                        label="⬇️ Download PDF",
                        data=img_bytes,
                        file_name="metrics_boxplot.pdf",
                        mime="application/pdf"
                    )
            
            st.plotly_chart(fig_box, use_container_width=True)
        else:
            st.info("📭 Select metrics above to generate box plot")
    ...
    with tab4:
        ...
        if st.session_state.solutions:
            ...
            for idx, res in results.items():
                sol_copy = st.session_state.solutions[idx].copy()
                sol_copy['metrics'] = res['metrics']
                solutions_with_metrics.append(sol_copy)
            ...
            col_export1, col_export2, col_export3 = st.columns(3)
            with col_export1:
                if st.button("📥 Export PNG", key="export_png_tab4"):
                    img_bytes = export_plot_to_image(fig_heatmap, format='png')
                    st.download_button(
                        label="⬇️ Download PNG",
                        data=img_bytes,
                        file_name=f"param_space_{heatmap_metric.replace(' ', '_')}.png",
                        mime="image/png"
                    )
            with col_export2:
                if st.button("📥 Export SVG", key="export_svg_tab4"):
                    img_bytes = export_plot_to_image(fig_heatmap, format='svg')
                    st.download_button(
                        label="⬇️ Download SVG",
                        data=img_bytes,
                        file_name=f"param_space_{heatmap_metric.replace(' ', '_')}.svg",
                        mime="image/svg+xml"
                    )
            with col_export3:
                if st.button("📥 Export PDF", key="export_pdf_tab4"):
                    img_bytes = export_plot_to_image(fig_heatmap, format='pdf')
                    st.download_button(
                        label="⬇️ Download PDF",
                        data=img_bytes,
                        file_name=f"param_space_{heatmap_metric.replace(' ', '_')}.pdf",
                        mime="application/pdf"
                    )
            
            st.plotly_chart(fig_heatmap, use_container_width=True)
        else:
            st.info("📭 Load solutions first to generate parameter space heatmap")
        ...
        if case_idx_match:
            ...
            if case_idx in results:
                ...
                with col1:
                    ...
                    # Export buttons for Cu
                    col_e1, col_e2 = st.columns(2)
                    with col_e1:
                        if st.button("📥 Cu PNG", key="export_c1_png"):
                            img_bytes = export_plot_to_image(fig_c1, format='png')
                            st.download_button(
                                label="⬇️ PNG",
                                data=img_bytes,
                                file_name=f"{selected_case_label}_Cu.png",
                                mime="image/png",
                                key="dl_c1_png"
                            )
                    with col_e2:
                        if st.button("📥 Cu SVG", key="export_c1_svg"):
                            img_bytes = export_plot_to_image(fig_c1, format='svg')
                            st.download_button(
                                label="⬇️ SVG",
                                data=img_bytes,
                                file_name=f"{selected_case_label}_Cu.svg",
                                mime="image/svg+xml",
                                key="dl_c1_svg"
                            )
                with col2:
                    st.plotly_chart(fig_c2, use_container_width=True)
                    # Export buttons for Ni
                    col_e1, col_e2 = st.columns(2)
                    with col_e1:
                        if st.button("📥 Ni PNG", key="export_c2_png"):
                            img_bytes = export_plot_to_image(fig_c2, format='png')
                            st.download_button(
                                label="⬇️ PNG",
                                data=img_bytes,
                                file_name=f"{selected_case_label}_Ni.png",
                                mime="image/png",
                                key="dl_c2_png"
                            )
                    with col_e2:
                        if st.button("📥 Ni SVG", key="export_c2_svg"):
                            img_bytes = export_plot_to_image(fig_c2, format='svg')
                            st.download_button(
                                label="⬇️ SVG",
                                data=img_bytes,
                                file_name=f"{selected_case_label}_Ni.svg",
                                mime="image/svg+xml",
                                key="dl_c2_svg"
                            )
        ...
        if not saved_runs:
            ...
            if runs_to_compare:
                ...
                if case_metrics_dict:
                    ...
                    col_export1, col_export2, col_export3 = st.columns(3)
                    with col_export1:
                        if st.button("📥 Export PNG", key="export_png_tab6_bar"):
                            img_bytes = export_plot_to_image(fig_saved_bar, format='png')
                            st.download_button(
                                label="⬇️ Download PNG",
                                data=img_bytes,
                                file_name="saved_runs_comparison.png",
                                mime="image/png"
                            )
                    with col_export2:
                        if st.button("📥 Export SVG", key="export_svg_tab6_bar"):
                            img_bytes = export_plot_to_image(fig_saved_bar, format='svg')
                            st.download_button(
                                label="⬇️ Download SVG",
                                data=img_bytes,
                                file_name="saved_runs_comparison.svg",
                                mime="image/svg+xml"
                            )
                    with col_export3:
                        if st.button("📥 Export PDF", key="export_pdf_tab6_bar"):
                            img_bytes = export_plot_to_image(fig_saved_bar, format='pdf')
                            st.download_button(
                                label="⬇️ Download PDF",
                                data=img_bytes,
                                file_name="saved_runs_comparison.pdf",
                                mime="application/pdf"
                            )
                ...
                if radar_metrics_dict:
                    ...
                    col_export1, col_export2, col_export3 = st.columns(3)
                    with col_export1:
                        if st.button("📥 Export PNG", key="export_png_tab6_radar"):
                            img_bytes = export_plot_to_image(fig_saved_radar, format='png')
                            st.download_button(
                                label="⬇️ Download PNG",
                                data=img_bytes,
                                file_name="saved_runs_radar.png",
                                mime="image/png"
                            )
                    with col_export2:
                        if st.button("📥 Export SVG", key="export_svg_tab6_radar"):
                            img_bytes = export_plot_to_image(fig_saved_radar, format='svg')
                            st.download_button(
                                label="⬇️ Download SVG",
                                data=img_bytes,
                                file_name="saved_runs_radar.svg",
                                mime="image/svg+xml"
                            )
                    with col_export3:
                        if st.button("📥 Export PDF", key="export_pdf_tab6_radar"):
                            img_bytes = export_plot_to_image(fig_saved_radar, format='pdf')
                            st.download_button(
                                label="⬇️ Download PDF",
                                data=img_bytes,
                                file_name="saved_runs_radar.pdf",
                                mime="application/pdf"
                            )
