...
def plot_flux_vs_gradient(solution, time_index,
                          ...
                          label_fontsize=18, title_fontsize=18, scatter_alpha=0.6,
    ...
    axes[1].set_ylabel('J$_y$', fontsize=label_fontsize)
...
def plot_uphill_heatmap(solution, time_index, cmap='viridis', vmin=None, vmax=None,
    ...
    fig.subplots_adjust(left=0.02, right=0.83, top=0.88, bottom=0.12, wspace=0.30)
    Ly = solution['params']['Ly']
...
def plot_uphill_over_time(solution, figsize=(8,3), linewidth=1.6, marker_size=6,
    ...
    # -----------------------------------------------------------------
    # NEW: Build a DataFrame and save as CSV
    # -----------------------------------------------------------------
    df_uphill = pd.DataFrame({
        "time_s": times,
        "max_pos_JdotGrad_Cu": max_pos_cu_list,
        "max_pos_JdotGrad_Ni": max_pos_ni_list,
        "frac_uphill_Cu": frac_cu_list,
        "frac_uphill_Ni": frac_ni_list,
        "avg_pos_JdotGrad_Cu": avg_pos_cu_list,
        "avg_pos_JdotGrad_Ni": avg_pos_ni_list,
        "total_intensity_Cu": total_intensity_cu_list,
        "total_intensity_Ni": total_intensity_ni_list,
    })
    Ly = solution['params']['Ly']
    base_filename = f"uphill_over_time_ly_{Ly:.1f}_ccu_{solution['params']['C_Cu']:.1e}_cni_{solution['params']['C_Ni']:.1e}"
    csv_path = os.path.join(output_dir, f"{base_filename}.csv")
    os.makedirs(output_dir, exist_ok=True)
    df_uphill.to_csv(csv_path, index=False)
    ...
    return fig, fig_avg, fig_frac, fig_intensity, base_filename, csv_path
...
def plot_polar_concentration_heatmap(concs, lys, times, title, cmap='viridis', output_dir="figures"):
    fig = plt.figure(figsize=(8,8))
    ax = fig.add_subplot(111, polar=True)
    # Theta edges for sectors
    theta_edges = np.linspace(0, 2 * np.pi, len(lys) + 1)
    # Normalize r to 0-1, with r=1 corresponding to max(times)=200s
    max_t = 200.0
    r_edges = np.linspace(0, 1, len(times) + 1)
    Theta, R = np.meshgrid(theta_edges, r_edges)
    # Z shape (len(times), len(lys))
    Z = concs.T
    im = ax.pcolormesh(Theta, R, Z, cmap=cmap, shading='auto')
    # Set theta ticks and labels
    theta_ticks = (theta_edges[:-1] + theta_edges[1:]) / 2
    ax.set_thetagrids(theta_ticks * 180 / np.pi, labels=[f"{ly:.0f}" for ly in lys])
    ax.set_theta_zero_location('N')
    ax.set_thetalim(0, 2 * np.pi)
    # Set r ticks as time labels
    r_ticks = np.linspace(0, 1, 5)
    ax.set_rticks(r_ticks)
    ax.set_rlabel_position(0)
    ax.set_yticklabels([f"{t * max_t:.0f} s" for t in r_ticks])
    # Colorbar
    fig.colorbar(im, ax=ax, label='Concentration (mol/cc)', format='%.1e')
    ax.set_title(title)
    # Save
    base_filename = f"polar_{title.lower().replace(' ', '_')}_ccu_{concs[0,0]:.1e}_cni_{concs[0,0]:.1e}"  # Approximate, adjust if needed
    os.makedirs(output_dir, exist_ok=True)
    plt.savefig(os.path.join(output_dir, f"{base_filename}.png"), dpi=300, bbox_inches='tight')
    plt.savefig(os.path.join(output_dir, f"{base_filename}.pdf"), bbox_inches='tight')
    plt.close()
...
def main():
    ...
    # New: Polar Heatmap for Center Concentrations
    st.subheader("Polar Heatmap of Center Concentrations over Ly and Time")
    show_polar = st.checkbox("Show Polar Heatmaps", value=False)
    if show_polar:
        with st.spinner("Interpolating and computing center concentrations..."):
            Ly_values = [30, 40, 50, 60, 70, 80, 90, 100]
            times = solution['times']
            max_t = 200.0
            t_idx = np.where(times <= max_t)[0]
            times = times[t_idx]
            cu_center = np.zeros((len(Ly_values), len(times)))
            ni_center = np.zeros((len(Ly_values), len(times)))
            for i, ly in enumerate(Ly_values):
                sol = load_and_interpolate_solution(solutions, params_list, ly, c_cu_target, c_ni_target)
                x_coords = sol['X'][:, 0]
                y_coords = sol['Y'][0, :]
                x_center_idx = np.argmin(np.abs(x_coords - sol['params']['Lx'] / 2))
                y_center_idx = np.argmin(np.abs(y_coords - ly / 2))
                for j, tidx in enumerate(t_idx):
                    cu_center[i, j] = sol['c1_preds'][tidx][x_center_idx, y_center_idx]
                    ni_center[i, j] = sol['c2_preds'][tidx][x_center_idx, y_center_idx]
            fig_cu, filename_cu = plot_polar_concentration_heatmap(cu_center, Ly_values, times, 'Cu Concentration at Center', cmap=cmap_cu)
            fig_ni, filename_ni = plot_polar_concentration_heatmap(ni_center, Ly_values, times, 'Ni Concentration at Center', cmap=cmap_ni)
            st.pyplot(fig_cu)
            st.download_button("Download Cu Polar as PNG", data=open(os.path.join("figures", f"{filename_cu}.png"), "rb").read(), file_name=f"{filename_cu}.png", mime="image/png")
            st.download_button("Download Cu Polar as PDF", data=open(os.path.join("figures", f"{filename_cu}.pdf"), "rb").read(), file_name=f"{filename_cu}.pdf", mime="application/pdf")
            st.pyplot(fig_ni)
            st.download_button("Download Ni Polar as PNG", data=open(os.path.join("figures", f"{filename_ni}.png"), "rb").read(), file_name=f"{filename_ni}.png", mime="image/png")
            st.download_button("Download Ni Polar as PDF", data=open(os.path.join("figures", f"{filename_ni}.pdf"), "rb").read(), file_name=f"{filename_ni}.pdf", mime="application/pdf")
    # Parameter Sweep Curves
    ...
    # Summary Table for All Solutions
    st.subheader("Summary Table of Uphill Metrics Across Solutions")
    summary_time_index = st.slider("Select Time Index for Summary", 0, len(solution['times'])-1, len(solution['times'])-1)
    df_summary = compute_summary_dataframe(solutions + sweep_solutions, time_index_for_summary=summary_time_index)
    ...
    df_summary = compute_summary_dataframe(solutions, time_index_for_summary=sweep_time_index)
    st.dataframe(df_summary.style.format({
        "Ly (μm)": "{:.1f}",
        "C_Cu": "{:.1e}",
        "C_Ni": "{:.1e}",
    ...
    st.download_button(
        ...
        data=csv_summary,
        file_name=f"uphill_summary_t_{solutions[0]['times'][sweep_time_index]:.1f}s.csv",
        mime="text/csv"
    )
