...
def main():
    ...
    # Compute metric per solution
    metrics = []
    lys_valid = []

    for sol in solutions:
        if 'J1_preds' not in sol:
            continue  # Skip if fluxes not computed
        try:
            (_, _, _, _, max_cu, max_ni, frac_cu, frac_ni, _, _, total_cu, total_ni) = detect_uphill(sol, opt_time_idx)
            value = {
                "Max positive J·∇c (Cu)": max_cu,
                "Max positive J·∇c (Ni)": max_ni,
                "Total uphill intensity (Cu)": total_cu,
                "Total uphill intensity (Ni)": total_ni,
                "Uphill fraction (Cu)": frac_cu,
                "Uphill fraction (Ni)": frac_ni
            }[metric]
    ...
    if metrics:
        ...
        st.success(f"**Optimal \(L_y\) = {Ly_opt:.1f} μm** → {metric} = **{val_opt:.3e}** ({extremum})")
        
        # Optional: Interpolate and show result
        if st.button(f"Show Result at \(L_y\) = {Ly_opt:.1f} μm"):
            opt_sol = load_and_interpolate_solution(solutions, params_list, Ly_opt, 
                                                    solutions[0]['params']['C_Cu'], solutions[0]['params']['C_Ni'])
            # Recompute fluxes
            J1, J2, grad1, grad2 = compute_fluxes_and_grads(opt_sol['c1_preds'], opt_sol['c2_preds'],
                                                            opt_sol['X'], opt_sol['Y'], opt_sol['params'])
