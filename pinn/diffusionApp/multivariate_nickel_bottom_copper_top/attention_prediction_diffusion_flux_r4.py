...
# =============================================================================
# ENHANCED UPHILL HEATMAP (with improved padding, spacing, and layout)
# =============================================================================
def plot_uphill_heatmap(
        solution, time_index, cmap='viridis', vmin=None, vmax=None,
        figsize=(14, 6), colorbar=True, cbar_label='J·∇c',
        label_fontsize=13, title_fontsize=15, downsample=1,
        output_dir="figures"):
    """
    Enhanced uphill-diffusion heat-map with:
      • larger figure & equal-aspect cells
      • generous wspace / hspace
      • label/title padding
      • colour-bar that never overlaps the maps
      • optional down-sampling for speed
    """
    # ------------------------------------------------------------------ #
    # 1. Extract coordinates (handle 1-D or 2-D X/Y)
    # ------------------------------------------------------------------ #
    x_coords = solution['X'][:, 0] if solution['X'].ndim == 2 else solution['X']
    y_coords = solution['Y'][0, :] if solution['Y'].ndim == 2 else solution['Y']
    ...
    # ------------------------------------------------------------------ #
    # 2. Compute uphill products (positive J·∇c only)
    # ------------------------------------------------------------------ #
    (uphill_cu, uphill_ni,
     uphill_prod_cu_pos, uphill_prod_ni_pos,
     max_pos_cu, max_pos_ni,
    ...
    # ------------------------------------------------------------------ #
    # 3. Down-sample for speed (keeps the same physical extent)
    # ------------------------------------------------------------------ #
    ...
    fig, axes = plt.subplots(
        1, 2, figsize=figsize,
        constrained_layout=False)          # we control layout ourselves

    # ---- Cu map ------------------------------------------------------- #
    im1 = axes[0].imshow(
        z1, origin='lower', aspect='equal',
    ...
    axes[1].set_ylabel('')                     # y-label only on the left
    ...
    if colorbar:
        cbar = fig.colorbar(
            im2, ax=axes.ravel().tolist(),
            orientation='vertical', fraction=0.046, pad=0.08, shrink=0.8)
        cbar.set_label(cbar_label, fontsize=label_fontsize, labelpad=10)
        cbar.ax.tick_params(labelsize=label_fontsize-2)

    # ---- Global title & fine-tune spacing ----------------------------- #
    fig.suptitle(
        f'Uphill Diffusion (positive J·∇c) @ t = {t_val:.2f} s',
    ...
    Ly = solution['params']['Ly']
    base_filename = (
        f"uphill_heatmap_t_{t_val:.1f}_ly_{Ly:.1f}_"
        f"ccu_{solution['params']['C_Cu']:.1e}_cni_{solution['params']['C_Ni']:.1e}"
    )
    os.makedirs(output_dir, exist_ok=True)
    for ext in ('.png', '.pdf'):
        fig.savefig(
...
def plot_uphill_over_time(solution, figsize=(8,3), linewidth=1.6, marker_size=6,
    ...
    ax_avg.set_ylabel('Avg positive J·∇c', fontsize=_fontsize)
...
def main():
    ...
    # ------------------------------------------------------------------ #
    # 3. Sort unique parameters for UI
    # ------------------------------------------------------------------ #
    lys = sorted(set(lys))
    ...
    if use_custom_params:
        ly_target = st.number_input(
            "Custom Ly (μm)",
            min_value=30.0, max_value=120.0, value=ly_choice, step=0.1, format="%.1f"
        )
        c_cu_target = st.number_input(
            "Custom C_Cu (mol/cc)",
            min_value=0.0, max_value=2.9e-3, value=max(c_cu_choice, 1.5e-3), step=0.1e-3, format="%.1e"
        )
        c_ni_target = st.number_input(
            "Custom C_Ni (mol/cc)",
            min_value=0.0, max_value=1.8e-3, value=max(c_ni_choice, 1.0e-4), step=0.1e-4, format="%.1e"
        )
    else:
        ly_target, c_cu_target, c_ni_target = ly_choice, c_cu_choice, c_ni_choice

    # ------------------------------------------------------------------ #
    # 5. GLOBAL VISUALISATION SETTINGS
    # ------------------------------------------------------------------ #
    ...
    # ------------------------------------------------------------------ #
    # 6. Load / interpolate the **single** solution
    # ------------------------------------------------------------------ #
    try:
    ...
    # compute fluxes / gradients if missing
    if 'J1_preds' not in solution or 'grad_c1_y' not in solution:
        ...
        try:
            J1, J2, grad_c1, grad_c2 = compute_fluxes_and_grads(
                ...
                solution['X'], solution['Y'], solution['params']
            )
    ...
    time_index = st.slider(
        "Select Time Index for Heatmaps",
        0, len(solution['times'])-1, len(solution['times'])-1
    )
    ...
    for ext in ('png','pdf'):
        ...
        st.download_button(
            f"Download 2D Plot as {ext.upper()}",
            data=open(path,"rb").read(),
            file_name=f"{filename_2d}.{ext}",
            mime=f"image/{ext}" if ext=='png' else "application/pdf"
        )
    ...
    if time_indices:
        fig_curves, filename_curves = plot_centerline_curves(
            ...
            tick_label_size=tick_label_size, legend_loc=legend_loc,
            curve_colormap=curve_colormap, axis_linewidth=axis_linewidth,
            tick_major_width=tick_major_width, tick_major_length=tick_major_length,
            fig_width=fig_width, fig_height=fig_height,
            curve_linewidth=curve_linewidth, grid_alpha=grid_alpha,
        ...
        for ext in ('png','pdf'):
            ...
            st.download_button(
                f"Download Centerline Plot as {ext.upper()}",
                data=open(path,"rb").read(),
                file_name=f"{filename_curves}.{ext}",
                mime=f"image/{ext}" if ext=='png' else "application/pdf"
            )

    # ------------------------------------------------------------------ #
    # 10. UPHILL DIFFUSION ANALYSIS (single solution)
    # ------------------------------------------------------------------ #
    st.subheader("Uphill Diffusion Analysis")

    # 10.1 Flux vs Gradient scatter
    ...
    for ext in ('png','pdf'):
        ...
        st.download_button(
            f"Download Flux vs Gradient as {ext.upper()}",
            data=buf.read(),
            file_name=f"{filename_fg}.{ext}",
            mime=f"image/{ext}" if ext=='png' else "application/pdf"
        )

    # 10.2 Uphill heat-maps
    ...
    for ext in ('png','pdf'):
        ...
        st.download_button(
            f"Download Uphill Heatmaps as {ext.upper()}",
            data=buf.read(),
            file_name=f"{filename_hm}.{ext}",
            mime=f"image/{ext}" if ext=='png' else "application/pdf"
        )
    st.markdown(f"- **Max (positive) J·∇c (Cu):** {max_cu:.3e}")
    st.markdown(f"- **Max (positive) J·∇c (Ni):** {max_ni:.3e}")
    st.markdown(f"- **Avg (positive) J·∇c (Cu):** {avg_cu:.3e}")
    st.markdown(f"- **Avg (positive) J·∇c (Ni):** {avg_ni:.3e}")
    st.markdown(f"- **Total intensity (Cu):** {tot_cu:.3e}")
    st.markdown(f"- **Total intensity (Ni):** {tot_ni:.3e}")
    st.markdown(f"- **Uphill fraction (Cu):** {frac_cu*100:.2f}%")
    st.markdown(f"- **Uphill fraction (Ni):** {frac_ni*100:.2f}%")

    # 10.3 Temporal evolution of uphill metrics
    st.markdown("**3) Temporal Evolution of Uphill Metrics**")
    fig_max, fig_avg, fig_frac, fig_int, filename_time = plot_uphill_over_time(
        solution,
        figsize=(fig_width, fig_height/2),
        linewidth=linewidth,
        marker_size=max(4, int(marker_size/2))
    )
    for f, name in zip([fig_max, fig_avg, fig_frac, fig_int],
                       ["_max", "_avg", "_frac", "_intensity"]):
        st.pyplot(f)
        for ext in ('png','pdf'):
            buf = fig_to_bytes(f, fmt=ext)
            st.download_button(
                f"Download {name[1:].capitalize()} Evolution as {ext.upper()}",
                data=buf.read(),
                file_name=f"{filename_time}{name}.{ext}",
                mime=f"image/{ext}" if ext=='png' else "application/pdf"
            )
    ...
    # ---- custom extra parameter sets ---------------------------------------
    with st.expander("Add Custom Parameter Combinations for Sweep"):
        n_custom = st.number_input("Number of Custom Parameter Sets", 0, 5, 0, 1)
        custom_params = []
        for i in range(n_custom):
            st.write(f"**Custom Set {i+1}**")
            ly_c = st.number_input(f"Ly (μm) {i+1}", 30.0, 120.0, ly_choice, 0.1, format="%.1f", key=f"ly_c_{i}")
            cu_c = st.number_input(f"C_Cu (mol/cc) {i+1}", 0.0, 2.9e-3, max(c_cu_choice,1.5e-3), 0.1e-3, format="%.1e", key=f"cu_c_{i}")
            ni_c = st.number_input(f"C_Ni (mol/cc) {i+1}", 0.0, 1.8e-3, max(c_ni_choice,1.0e-4), 0.1e-4, format="%.1e", key=f"ni_c_{i}")
            custom_params.append((ly_c, cu_c, ni_c))

    # ---- select exact solutions ------------------------------------------------
    ...
    for ly, c_cu, c_ni in selected_params:
        try:
            ...
            if 'J1_preds' not in sol:
                J1, J2, grad_c1, grad_c2 = compute_fluxes_and_grads(
                    sol['c1_preds'], sol['c2_preds'],
                    sol['X'], sol['Y'], sol['params']
                )
                sol['J1_preds'] = J1
                sol['J2_preds'] = J2
    ...
    # ---- plot sweep -----------------------------------------------------------
    sweep_time_index = st.slider(
        "Select Time Index for Sweep",
        0, len(solution['times'])-1, len(solution['times'])-1
    )
    if sweep_solutions:
        fig_sweep, filename_sweep = plot_parameter_sweep(
            sweep_solutions, sweep_params_list, selected_params,
            sweep_time_index, sidebar_metric=sidebar_metric,
            label_size=label_size, title_size=title_size,
            tick_label_size=tick_label_size, legend_loc=legend_loc,
            ...
            grid_alpha=grid_alpha, grid_linestyle=grid_linestyle,
            legend_frameon=legend_frameon, legend_framealpha=legend_framealpha
        )
        st.pyplot(fig_sweep)
        for ext in ('png','pdf'):
            ...
            st.download_button(
                f"Download Sweep Plot as {ext.upper()}",
                data=open(path,"rb").read(),
                file_name=f"{filename_sweep}.{ext}",
                mime=f"image/{ext}" if ext=='png' else "application/pdf"
            )
    else:
        st.info("No solutions selected for the parameter sweep.")

    # ------------------------------------------------------------------ #
    # 12. SUMMARY TABLE (all loaded solutions)
    # ------------------------------------------------------------------ #
    st.subheader("Summary Table (all loaded solutions)")
    summary_time_idx = st.slider(
        "Time index for summary metrics",
        0, len(solutions[0]['times'])-1, len(solutions[0]['times'])-1
    )
    ...
    # 13. Final note
    # ------------------------------------------------------------------ #
    st.success("All visualisations generated! Files are saved in the `figures/` folder.")
