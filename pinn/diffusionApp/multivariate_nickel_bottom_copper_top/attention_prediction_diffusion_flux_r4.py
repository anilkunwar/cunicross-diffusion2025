...
def plot_uphill_heatmap(solution, timeindex, cmap=plt.cm.viridis, vmin=None, vmax=None, 
                      figsize=(14, 6), colorbar=True, cbarlabel="Jc", 
                      labelfontsize=14, titlefontsize=16, downsample=1, outputdir="figures"):

    # Access coordinates safely
    xcoords = solution["X"][0, :] if solution["X"].ndim == 2 else solution["X"]
    ycoords = solution["Y"][:, 0] if solution["Y"].ndim == 2 else solution["Y"]
    tval = solution["times"][timeindex]

    # Obtain uphill diffusion detection results for the given time index
    (uphillcu, uphillni, uphillprodcupos, uphillprodnipos, maxposcu, maxposni, 
     fraccu, fracni, avgposcu, avgposni, totalintensitycu, totalintensityni) = detect_uphillsolution(solution, timeindex)

    # Downsample data for faster plotting if needed
    z1 = uphillprodcupos[::downsample, ::downsample]
    z2 = uphillprodnipos[::downsample, ::downsample]
    xds = xcoords[::downsample]
    yds = ycoords[::downsample]
    ...
    im1 = axes[0].imshow(z1, origin='lower', aspect='auto',
                         extent=(xds[0], xds[-1], yds[0], yds[-1]),
                         cmap=cmap, vmin=vmin, vmax=vmax)
    axes[0].set_title(f"Cu Uphill max {maxposcu:.3e}, avg {avgposcu:.3e}", fontsize=titlefontsize)
    axes[0].set_xlabel("x [m]", fontsize=labelfontsize, labelpad=12)
    axes[0].set_ylabel("y [m]", fontsize=labelfontsize, labelpad=12)
    axes[0].tick_params(axis='both', which='major', labelsize=labelfontsize-2)
    ...
    im2 = axes[1].imshow(z2, origin='lower', aspect='auto',
                         extent=(xds[0], xds[-1], yds[0], yds[-1]),
                         cmap=cmap, vmin=vmin, vmax=vmax)
    axes[1].set_title(f"Ni Uphill max {maxposni:.3e}, avg {avgposni:.3e}", fontsize=titlefontsize)
    axes[1].set_xlabel("x [m]", fontsize=labelfontsize, labelpad=12)
    axes[1].set_ylabel("y [m]", fontsize=labelfontsize, labelpad=12)
    axes[1].tick_params(axis='both', which='major', labelsize=labelfontsize-2)
    ...
    if colorbar:
        ...
        cbar.set_label(cbarlabel, fontsize=labelfontsize)
        cbar.ax.tick_params(labelsize=labelfontsize-2)

    # Adjust layout and spacing
    fig.suptitle(f"Uphill Diffusion positive Jc t={tval:.2f}s", fontsize=titlefontsize + 2)
    ...
    # Save figures with high DPI for publication quality
    os.makedirs(outputdir, exist_ok=True)
    basefilename = f"uphillheatmap_t{tval:.1f}_Ly{solution['params']['Ly']:.1f}_CCu{solution['params']['CCu']:.1e}_CNi{solution['params']['CNi']:.1e}"

    plt.savefig(os.path.join(outputdir, basefilename + ".png"), dpi=300, bbox_inches='tight')
    plt.savefig(os.path.join(outputdir, basefilename + ".pdf"), bbox_inches='tight')
    plt.close(fig)

    # Return figure and relevant data for further use if needed
    return fig, maxposcu, maxposni, fraccu, fracni, avgposcu, avgposni, totalintensitycu, totalintensityni, basefilename
