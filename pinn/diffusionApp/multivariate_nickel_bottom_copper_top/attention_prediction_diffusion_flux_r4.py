...
def plot_uphill_heatmap(solution, time_index, cmap='viridis', vmin=None, vmax=None,
                        ...
                        cbar_pad=0.04, output_dir="figures"):
    x_coords = solution['X'][:,0] if solution['X'].ndim == 2 else solution['X']
    y_coords = solution['Y'][0,:] if solution['Y'].ndim == 2 else solution['Y']
    t_val = solution['times'][time_index]
    (uphill_cu, uphill_ni,
...
def plot_uphill_over_time(solution, figsize=(8,3), linewidth=1.6, marker_size=6,
    ...
    fig.savefig(os.path.join(output_dir, f"{base_filename}_max.pdf"), bbox_inches='tight')
    plt.close(fig)
    fig_avg.savefig(os.path.join(output_dir, f"{base_filename}_avg.png"), dpi=300, bbox_inches='tight')
    fig_avg.savefig(os.path.join(output_dir, f"{base_filename}_avg.pdf"), bbox_inches='tight')
    plt.close(fig_avg)
    fig_frac.savefig(os.path.join(output_dir, f"{base_filename}_frac.png"), dpi=300, bbox_inches='tight')
    fig_frac.savefig(os.path.join(output_dir, f"{base_filename}_frac.pdf"), bbox_inches='tight')
    plt.close(fig_frac)
    fig_intensity.savefig(os.path.join(output_dir, f"{base_filename}_intensity.png"), dpi=300, bbox_inches='tight')
    fig_intensity.savefig(os.path.join(output_dir, f"{base_filename}_intensity.pdf"), bbox_inches='tight')
...
def main():
    ...
    if use_custom_params:
        ...
        c_cu_target = st.number_input("Custom C_Cu (mol/cc)", min_value=0.0, max_value=2.9e-3, value=max(c_cu_choice, 1.5e-3), step=0.1e-3, format="%.1e")
        c_ni_target = st.number_input("Custom C_Ni (mol/cc)", min_value=0.0, max_value=1.8e-3, value=max(c_ni_choice, 1.0e-4), step=0.1e-4, format="%.1e")
    else:
    ...
    cbar_pad = st.slider(
        "Color-bar padding (distance from plot)",
        min_value=0.0, max_value=0.20, value=0.04, step=0.005,
        help="Increase to push the color-bar farther away from the heatmaps."
    )
    ...
    st.pyplot(fig_2d)
    st.download_button("Download 2D Plot as PNG", data=open(os.path.join("figures", f"{filename_2d}.png"), "rb").read(), file_name=f"{filename_2d}.png", mime="image/png")
    st.download_button("Download 2D Plot as PDF", data=open(os.path.join("figures", f"{filename_2d}.pdf"), "rb").read(), file_name=f"{filename_2d}.pdf", mime="application/pdf")
    ...
    if time_indices:
        ...
        st.download_button("Download Centerline Plot as PNG", data=open(os.path.join("figures", f"{filename_curves}.png"), "rb").read(), file_name=f"{filename_curves}.png", mime="image/png")
        st.download_button("Download Centerline Plot as PDF", data=open(os.path.join("figures", f"{filename_curves}.pdf"), "rb").read(), file_name=f"{filename_curves}.pdf", mime="application/pdf")

    st.subheader("Parameter Sweep")
    if st.button("Run Parameter Sweep"):
        selected_params = [(ly, c_cu, c_ni) for ly in lys for c_cu in c_cus for c_ni in c_nis]
        fig_sweep, filename_sweep = plot_parameter_sweep(
            solutions, params_list, selected_params, time_index,
            sidebar_metric=sidebar_metric, label_size=label_size, title_size=title_size,
            tick_label_size=tick_label_size, legend_loc=legend_loc, curve_colormap=curve_colormap,
            axis_linewidth=axis_linewidth, tick_major_width=tick_major_width,
            tick_major_length=tick_major_length, fig_width=fig_width, fig_height=fig_height,
            curve_linewidth=curve_linewidth, grid_alpha=grid_alpha, grid_linestyle=grid_linestyle,
            legend_frameon=legend_frameon, legend_framealpha=legend_framealpha
        ...
        st.pyplot(fig_sweep)
        st.download_button("Download Sweep PNG", data=fig_to_bytes(fig_sweep, 'png').read(), file_name=f"{filename_sweep}.png", mime="image/png")
        st.download_button("Download Sweep PDF", data=fig_to_bytes(fig_sweep, 'pdf').read(), file_name=f"{filename_sweep}.pdf", mime="application/pdf")

    st.subheader("Summary Table of Uphill Diffusion Metrics")
    summary_time_index = st.slider("Time Index for Summary", 0, len(solution['times'])-1, 0)
    df = compute_summary_dataframe(solutions, summary_time_index)
    ...
    st.subheader("Find Optimal Domain Height (Ly)")
    metric = st.selectbox("Optimization Metric", ["total_intensity_Cu", "max_pos_JdotGrad_Cu", "uphill_frac_Cu"], index=0)
    if st.button("Find Optimal Ly"):
        df_opt = compute_summary_dataframe(solutions, summary_time_index)
        opt_row = df_opt.loc[df_opt[metric].idxmax()]
        st.write(f"**Optimal Ly = {opt_row['Ly (μm)']:.1f} μm** for max {metric}")
        st.write(opt_row)
