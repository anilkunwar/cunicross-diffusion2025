...
=====================================================================
Features:
- Enhanced solution loader (filename parsing, tensor conversion, standardization)
...
# =============================================
# ENHANCED SOLUTION LOADER (ROBUST FILE HANDLING)
# =============================================
class EnhancedSolutionLoader:
    """
    Loads PKL files from pinn_solutions directory, parsing filenames as fallback.
    Handles various data formats and standardizes them for interpolation.
    """
    ...
    def parse_filename(self, filename: str) -> Dict[str, Any]:
        """Parse parameters from filename as fallback."""
        params = {}
        ...
        # Extract C_Cu and C_Ni
        cu_match = re.search(r'C_Cu[_-]?([0-9.eE+-]+)', filename, re.IGNORECASE)
        if cu_match:
            params['C_Cu'] = float(cu_match.group(1))
        ni_match = re.search(r'C_Ni[_-]?([0-9.eE+-]+)', filename, re.IGNORECASE)
        if ni_match:
            params['C_Ni'] = float(ni_match.group(1))
        ...
        t_match = re.search(r't[_-]?max[_-]?([0-9.]+)', filename, re.IGNORECASE)
        if t_match:
            params['t_max'] = float(t_match.group(1))
        
        return params
    
    def _ensure_2d(self, arr):
        """Ensure array is 2D for interpolation."""
        if arr is None:
            return np.zeros((1, 1))
    ...
    def read_simulation_file(self, file_path: str) -> Optional[Dict]:
        """Read and standardize a single simulation file."""
        try:
            ...
            if isinstance(data, dict):
                ...
                # Extract coordinates
                standardized['X'] = data.get('X', None)
                standardized['Y'] = data.get('Y', None)
                standardized['coords_nd'] = data.get('coords_nd', None)
                
                # Extract fields (c1_preds = Cu, c2_preds = Ni)
                if 'c1_preds' in data:
                    standardized['c1_preds'] = data['c1_preds']
                if 'c2_preds' in data:
                    standardized['c2_preds'] = data['c2_preds']
                
                # Extract time array
                if 'times' in data:
                    standardized['times'] = data['times']
                
                # Handle history/snapshots format
                ...
                # Handle thickness history
                if 'thickness_history' in data:
                    standardized['thickness_history'] = data['thickness_history']
                
                # Fallback: parse parameters from filename
                if not standardized['params']:
                    parsed = self.parse_filename(os.path.basename(file_path))
                    standardized['params'].update(parsed)
                
                # Set defaults for missing required params
                params = standardized['params']
                params.setdefault('Ly', params.get('Ly', 60.0))
                params.setdefault('C_Cu', params.get('C_Cu', 1.5e-3))
                params.setdefault('C_Ni', params.get('C_Ni', 0.5e-3))
                params.setdefault('Lx', params.get('Lx', 60.0))
                params.setdefault('t_max', params.get('t_max', 200.0))
    ...
    def load_all_solutions(self, use_cache: bool = True, max_files: Optional[int] = None) -> Tuple[List, List, List, List, List, List[str]]:
        """Load all solutions and return standardized lists."""
        ...
        if not file_info:
            load_logs.append(f"No PKL files found in {self.solutions_dir}")
        ...
        for item in file_info:
            ...
            if sol:
                # Validate required keys for interpolation
                ...
                if all(k in sol for k in required):
                    ...
                    if isinstance(c1, list) and len(c1) > 0:
                        c1 = c1[0] if isinstance(c1[0], np.ndarray) else c1
                    if isinstance(c2, list) and len(c2) > 0:
                        c2 = c2[0] if isinstance(c2[0], np.ndarray) else c2
                    
                    if np.any(np.isnan(c1)) or np.any(np.isnan(c2)) or (np.all(c1==0) and np.all(c2==0)):
                        load_logs.append(f"{item['filename']}: Skipped (invalid/zero data)")
                        continue
                    ...
                    p = sol['params']
                    param_tuple = (p.get('Ly', 60.0), p.get('C_Cu', 1.5e-3), p.get('C_Ni', 0.5e-3))
                    params_list.append(param_tuple)
                    lys.append(p.get('Ly', 60.0))
                    c_cus.append(p.get('C_Cu', 1.5e-3))
                    c_nis.append(p.get('C_Ni', 0.5e-3))
                    
                    load_logs.append(f"{item['filename']}: Loaded. Ly={param_tuple[0]:.1f}, C_Cu={param_tuple[1]:.1e}, C_Ni={param_tuple[2]:.1e}")
                else:
                    missing = [k for k in required if k not in sol]
                    load_logs.append(f"{item['filename']}: Skipped - missing keys: {missing}")
            else:
        ...
        if not solutions:
            ...
            load_logs.append(f"Total: Loaded {len(solutions)} valid solutions.")
...
def compute_validation_metrics(interp_c1, interp_c2, gt_c1, gt_c2, x, y, t, params, weights=None) -> ValidationMetrics:
    ...
    # Uncertainty - FIXED: convert list to array
    if weights is not None and len(weights)>0:
        ...
        m.weight_entropy = float(-np.sum(weights_arr * np.log(weights_arr + eps)))
    if 'target_params' in params and 'source_params' in params:
        ...
        if sources:
            ...
            m.param_distance = float(min(dists)) if dists else 1.0
    # Overall score
...
def main():
    ...
    with st.sidebar:
        ...
        if st.button("📥 Load Solutions from pinn_solutions/", use_container_width=True):
            with st.spinner("Loading solutions..."):
                solutions, params_list, lys_raw, c_cus_raw, c_nis_raw, logs = st.session_state.loader.load_all_solutions()
                if not solutions:
                    ...
                    st.success(f"✅ Loaded {len(solutions)} solutions")
        ...
        if TRANSFORMERS_AVAILABLE:
            ...
            backend = st.selectbox("LLM Backend", ["GPT-2 (default)", "Qwen2-0.5B-Instruct", "Qwen2.5-0.5B-Instruct"], key="llm_backend_select")
            ...
            use_llm = st.checkbox("Enable LLM Parsing", value=True, key="llm_toggle")
        else:
            use_llm = False
        ...
        ly_input = st.number_input("Ly (μm)", 30.0, 120.0, 60.0, 0.1, key="ly_num")
        c_cu_input = st.number_input("C_Cu (mol/cc)", 0.0, 2.9e-3, 1.5e-3, 1e-5, format="%.1e", key="ccu_num")
        c_ni_input = st.number_input("C_Ni (mol/cc)", 0.0, 1.8e-3, 0.5e-3, 1e-5, format="%.1e", key="cni_num")

        st.subheader("🔬 Validation Settings")
        held_out_frac = st.slider("Held-out fraction", 0.1, 0.5, 0.2, 0.05, key="held_frac")
        physics_aware = st.checkbox("Physics-aware interpolation", value=True, key="phys_aware")
        optimize_fields = st.checkbox("Optimize fields (PDE refinement)", value=True, disabled=not physics_aware, key="opt_fields")
        ...
        if st.button("🚀 Run Validation", type="primary", use_container_width=True):
            ...
            with st.spinner("Running validation pipeline..."):
                ...
                params_list = st.session_state.params_list
    ...
    if not st.session_state.validation_results:
        st.info("👈 Load solutions and click 'Run Validation' to start.")
        return
    ...
    with tab3:
        case_idx = st.selectbox("Select case", list(results.keys()), format_func=lambda x: f"Case #{x}", key="case_select")
