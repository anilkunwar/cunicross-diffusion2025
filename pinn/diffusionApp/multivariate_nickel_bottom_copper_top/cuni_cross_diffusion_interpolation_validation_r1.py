...
# -*- coding: utf-8 -*-
"""
PHYSICS-INFORMED VALIDATION & UNCERTAINTY DASHBOARD FOR CU-NI INTERDIFFUSION
=============================================================================
- Natural language parameter extraction (regex + GPT-2/Qwen hybrid with confidence merging)
- Attention-based interpolation with spatial locality and physics-aware refinement
- Held-out PINN validation: MSE, MAE, R², SSIM, PDE residual, boundary errors, mass conservation
- Uncertainty quantification: weight entropy, parameter-space distance, ensemble variance
- Interactive visualizations: bar charts, radar plots, field comparisons, residual heatmaps
- Exportable diagnostic reports (JSON/CSV) with full metrics tables
- Publication-quality Matplotlib/Plotly figures with customizable styling
"""
import os
import re
import json
...
# =============================================
# 5. SOLUTION LOADER WITH VALIDATION
# =============================================
@st.cache_data
def load_solutions(solution_dir: str) -> Tuple[List, List, List, List, List, List[str]]:
    """Load and validate precomputed PINN solution files."""
    solutions, params_list, load_logs = [], [], []
    lys, c_cus, c_nis = [], [], []
    if not os.path.exists(solution_dir):
        load_logs.append(f"Directory {solution_dir} not found. Creating empty.")
        os.makedirs(solution_dir, exist_ok=True)
        ...
        if fname.endswith(".pkl"):
            try:
                ...
                if all(k in sol for k in required):
                    c1, c2 = sol['c1_preds'], sol['c2_preds']
                    if np.any(np.isnan(c1)) or np.any(np.isnan(c2)) or (np.all(c1==0) and np.all(c2==0)):
                        load_logs.append(f"{fname}: Skipped - Invalid/zero data.")
                        continue
                    solutions.append(sol)
                    p = (sol['params']['Ly'], sol['params']['C_Cu'], sol['params']['C_Ni'])
                    params_list.append(p)
                    lys.append(p[0]); c_cus.append(p[1]); c_nis.append(p[2])
                    load_logs.append(f"{fname}: Loaded. Ly={p[0]:.1f}, Cu={p[1]:.1e}, Ni={p[2]:.1e}")
                else:
                    load_logs.append(f"{fname}: Missing keys {[k for k in required if k not in sol]}")
            except Exception as e:
                load_logs.append(f"{fname}: Load failed - {e}")
    load_logs.append(f"Total Valid Solutions: {len(solutions)}")
    return solutions, params_list, lys, c_cus, c_nis, load_logs
    ...
    with st.sidebar:
        ...
        if st.button("🔄 Load Solutions & Run Validation"):
            with st.spinner("Loading solutions..."):
                solutions, params_list, lys_raw, c_cus_raw, c_nis_raw, logs = load_solutions(solution_dir)
                if not solutions:
...
def run_validation_pipeline(solutions: List[Dict], held_out_indices: List[int],
    ...
    for idx in held_out_indices:
        ...
        gt_c1 = gt_sol['c1_preds'][-1] if len(gt_sol['c1_preds']) > 0 else np.zeros_like(interp_c1)
