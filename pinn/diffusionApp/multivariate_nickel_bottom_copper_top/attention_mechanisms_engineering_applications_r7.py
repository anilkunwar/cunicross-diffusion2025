...
with st.sidebar:
    ...
    # New: Experimental info toggle and freeform question input
    st.header("📝 Transformer Input Control")
    include_experiment = st.checkbox(
        "Include Experimental Description in AI Prompt", value=True
    )
        ...
        with st.spinner("Computing attention weights and generating AI insights..."):
            ...
            with st.spinner("🤖 Generating AI-powered engineering insights..."):
                try:
                    ...
                    # Construct prompt based on freeform text question and experimental info checkbox
                    if general_text_question.strip():
                        context_part = EXPERIMENTAL_DESCRIPTION if include_experiment else ""
                        ai_prompt = f"""
                        You are a materials engineering expert.

                        QUESTION:
                        {general_text_question.strip()}
                        ...
                        exp_info = EXPERIMENTAL_DESCRIPTION if include_experiment else ""
                        if st.session_state.selected_faq:
                            ...
                            full_context = context + "\n\n" + exp_info if include_experiment else context
                            ai_prompt = f"""
                            ...
                            full_context = exp_info if include_experiment else ""
                            ai_prompt = f"""
