...
# ----------------------------------------------------------------------
# 6. Separate radar charts for Cu and Ni
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 7. Enhanced Streamlit UI with physics explanation
# ----------------------------------------------------------------------
def main():
    ...
    with radar_col1:
        fig_radar_cu, png_rcu, pdf_rcu = plot_radar_single(
            cu_row, "Cu", t_val, f"radar_cu_t{t_val:.0f}", LY_SPOKES
        )
    ...
    with radar_col2:
        fig_radar_ni, png_rni, pdf_rni = plot_radar_single(
            ni_row, "Ni", t_val, f"radar_ni_t{t_val:.0f}", LY_SPOKES
        )
    ...
    with st.expander("📊 Data Summary"):
        col1, col2 = st.columns(2)
        with col1:
            st.metric("Cu Range", f"{cu_mat.min():.2e} - {cu_mat.max():.2e} mol/cc")
            st.metric("Cu at t={t_val:.1f}s", f"{cu_row.mean():.2e} ± {cu_row.std():.2e} mol/cc")
        with col2:
            st.metric("Ni Range", f"{ni_mat.min():.2e} - {ni_mat.max():.2e} mol/cc")
            st.metric("Ni at t={t_val:.1f}s", f"{ni_row.mean():.2e} ± {ni_row.std():.2e} mol/cc")
