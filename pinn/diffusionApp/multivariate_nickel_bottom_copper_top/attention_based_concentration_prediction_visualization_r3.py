...
def plot_sunburst(data, title, cmap, vmin, vmax, log_scale, ly_dir, fname):
    ...
    ax.set_xticks(0.5 * (theta_edges[:-1] + theta_edges[1:]))
    ...
    plt.tight_layout()
    return fig, os.path.join(FIGURE_DIR, f"{fname}.png")

# ----------------------------------------------------------------------
# Radar chart
# ----------------------------------------------------------------------
def plot_radar(cu_row, ni_row, t_val):
...
def main():
    ...
    st.markdown("""
    ### 🔍 Physical Meaning
    - **Spokes (Angular Axis):** Different *film thicknesses (Ly)*.  
    - **Radial Length (Radius):** *Time evolution* of the center-point concentration.  
    """)
    ...
    c_cu_target = st.sidebar.number_input("Boundary Cu (mol/cc)", 1e-6, 5e-3, 1e-3, format="%.1e")
    c_ni_target = st.sidebar.number_input("Boundary Ni (mol/cc)", 1e-6, 5e-3, 1e-4, format="%.1e")
    ly_fraction = st.sidebar.selectbox("Ly fraction", {"Ly/2": 0.5, "Ly/3": 1/3, "Ly/4": 1/4})
    ly_dir = st.sidebar.radio("Ly Direction", ["top→bottom", "bottom→top"])

    cu_mat, ni_mat = build_sunburst_matrices(sols, params, interpolator, c_cu_target, c_ni_target, ly_fraction)

    col1, col2 = st.columns(2)
    with col1:
        fig_cu, _ = plot_sunburst(cu_mat, "Cu Concentration (Center)", cmap, 0, c_cu_target, log_scale, ly_dir, "sunburst_cu")
        st.pyplot(fig_cu)
    with col2:
        fig_ni, _ = plot_sunburst(ni_mat, "Ni Concentration (Center)", cmap, 0, c_ni_target, log_scale, ly_dir, "sunburst_ni")
        st.pyplot(fig_ni)
    ...
    fig_radar = plot_radar(cu_mat[t_idx], ni_mat[t_idx], t_val)
    st.pyplot(fig_radar)
