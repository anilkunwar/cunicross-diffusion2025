...
def main():
    ...
    if selected_session == "Create New Session":
        ...
        with st.spinner("🔄 Computing sunburst matrices with attention interpolation..."):
    ...
    with col1:
        fig_cu, png_cu, pdf_cu = plot_sunburst(
            ...
            ly_dir, f"sunburst_cu_{session_id}", times, LY_SPOKES
        )
        st.pyplot(fig_cu)
    ...
    with col2:
        fig_ni, png_ni, pdf_ni = plot_sunburst(
            ...
            ly_dir, f"sunburst_ni_{session_id}", times, LY_SPOKES
        )
        st.pyplot(fig_ni)
