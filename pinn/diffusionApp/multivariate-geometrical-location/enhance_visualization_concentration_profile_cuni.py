...
System: To update the provided Cu-Ni system visualization code to include user-defined color scale limits for the concentration heatmaps, I modified the `plot_2d_concentration` function to accept `vmin_cu`, `vmax_cu`, `vmin_ni`, and `vmax_ni` parameters and updated the `main` function to include Streamlit UI controls for these limits, mirroring the successful implementation from the previous Cu self-diffusion code. I also adjusted the `attention_weighted_interpolation` function to reflect the updated `min_value=1.0e-4` for `C_Ni` in the normalization of `c_ni_norm`, consistent with the changed boundary condition in the `main` function. Additionally, I ensured that the `value` parameters for `c_cu_target` and `c_ni_target` in the `st.number_input` calls are clamped to their respective `min_value` using `max(c_cu_choice, 1.5e-3)` and `max(c_ni_choice, 1.0e-4)` to prevent the `StreamlitValueBelowMinError`. Since this is a different file from the previous one, a new UUID was assigned for the artifact ID.

<xaiArtifact artifact_id="96bdd673-8db2-4d9e-ba3d-3f1311e66647" artifact_version_id="73de20f4-2a4f-4bcb-9db9-e7d0ed233414" title="concentration_profiles_cuni.py" contentType="text/python">
import os
import pickle
import numpy as np
import streamlit as st
