...
def generate_and_save_solution(_model, times, param_set, output_dir):
    ...
    solution_filename = os.path.join(output_dir, 
        f"solution_ly_{param_set['Ly']:.1f}_ccu_{param_set['C_Cu']:.1e}_cni_{param_set['C_Ni']:.1e}.pkl")
...
def main():
    ...
    C_Cu = st.sidebar.selectbox("C_Cu (mol/cc)", [1.5e-3, 1.9e-3, 2.5e-3, 2.9e-3], index=0)
    C_Ni = st.sidebar.selectbox("C_Ni (mol/cc)", [4.0e-4, 8.0e-4, 1.2e-3, 1.8e-3], index=0)
    ...
    if st.button("Train PINN Model"):
        ...
        loss_plot = plot_losses(loss_history, Ly, C_Cu, C_Ni, output_dir)
        profile_plot = plot_2d_profiles(solution, -1, output_dir)
        gradient_plot = plot_side_gradients(solution, model, -1, output_dir)
        
        st.success("Training complete!")
        st.image(loss_plot)
        st.image(profile_plot)
        st.image(gradient_plot)
        
        files = [solution_filename, loss_plot, profile_plot, gradient_plot]
        zip_file = create_zip_file(files, output_dir)
        with open(zip_file, 'rb') as f:
            st.download_button("Download All", f, "pinn_solutions.zip", "application/zip")
        
        for file in files:
            with open(file, 'rb') as f:
                st.download_button(os.path.basename(file), f, os.path.basename(file))
