...
def load_and_visualize_solution(D11, D12, D21, D22, Lx, t_max, ly_target, output_dir="pinn_solutions"):
    ...
    if exact_match_indices.size > 0:
        ...
        try:
            solution = attention_weighted_interpolation(
                valid_solutions, valid_lys, ly_target, sigma=5.0
            )
            st.info(f"Interpolated using {len(valid_solutions)} nearest solutions")
        except Exception as e:
