...
def train_and_save_solutions(D11, D12, D21, D22, Lx, t_max, epochs, output_dir="pinn_solutions"):
    ...
    for idx, param_set in enumerate(params):
        ...
        with st.spinner(f"Training with Ly={param_set['Ly']:.1f} μm..."):
            ...
            # Unique filename including all parameters
...
def load_and_visualize_solution(output_dir="pinn_solutions"):
    # Filter files for current parameter set
    solution_files = [f for f in os.listdir(output_dir) if f.startswith('solution_ly_') and f.endswith('.pkl')]
    solution_files.sort(key=lambda x: float(x.split('_ly_')[1].split('_d11_')[0]))
    
    if not solution_files:
        st.error("No solutions found. Please train models first.")
        return
    
    selected_ly = st.selectbox(
        "Select Ly (μm)",
        [float(f.split('_ly_')[1].split('_d11_')[0]) for f in solution_files],
        format_func=lambda ly: f"Ly = {ly:.1f} μm"
    )
    
    # Find the file matching selected Ly and current parameters
    D11 = st.session_state.get('D11', 0.006)
    D12 = st.session_state.get('D12', 0.00427)
    D21 = st.session_state.get('D21', 0.003697)
    D22 = st.session_state.get('D22', 0.0054)
    Lx = st.session_state.get('Lx', 60.0)
    t_max = st.session_state.get('t_max', 200.0)
    
    filename = os.path.join(output_dir, 
        f"solution_ly_{selected_ly:.1f}_d11_{D11:.6f}_d12_{D12:.6f}_d21_{D21:.6f}_d22_{D22:.6f}_lx_{Lx:.1f}_tmax_{t_max:.1f}.pkl")
    
    try:
        with open(filename, 'rb') as f:
            solution = pickle.load(f)
    except FileNotFoundError:
        st.error(f"Solution for Ly={selected_ly:.1f} μm with current parameters not found. Please train models with these settings.")
        return
    
    X, Y, c1_preds, c2_preds, times = (
...
if __name__ == "__main__":
    st.title("PINN Sweep for Varying Ly")
    ...
    with st.sidebar:
        ...
        # Store parameters in session state for visualization
    ...
    with col2:
        if st.button("Visualize Solutions"):
            st.session_state.show_visualization = True
    
    if st.session_state.get('show_visualization', False):
        load_and_visualize_solution(output_dir)
